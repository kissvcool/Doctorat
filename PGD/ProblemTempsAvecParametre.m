...
%% Premier membre
    IntHH = IntegrLine(h_q, h_q,s0,ds);
    Premier_g   = f_q' * K * f_q * IntHH;
    Premier_gp  = f_q' * C * f_q * IntHH;
    Premier_gpp = f_q' * M * f_q * IntHH;
    ...
    for t=1:size( HistF,2 )
        ...
        % Second membre
            ...
            for i=1:(m-1)
                ...
                f_k_q = HistMf(:,i);
                h_k_q = ;
                sum = sum + (f_q' * K * f_k_q) * IntegrLine(h_k_q,  h_q,s0,ds) * g_k_q(t);
            end
            Second = Second - sum;
            ...
            for i=1:(m-1)
                ...
                f_k_q = HistMf(:,i);
                h_k_q = ;
                sum = sum + (f_q' * C * f_k_q) * IntegrLine(h_k_q,  h_q,s0,ds) * g_kp_q(t);
            end
            Second = Second - sum;
            ...
            for i=1:(m-1)
                ...
                f_k_q = HistMf(:,i);
                h_k_q = ;
                sum = sum + (f_q' * M * f_k_q) * IntegrLine(h_k_q,  h_q,s0,ds) * g_kpp_q(t);
            end
            Second = Second - sum;
